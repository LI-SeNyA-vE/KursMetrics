...
func main() {
	...
	go func() {
		...
		funcAgent.SendingBatchMetric(gaugeMetrics, counterMetrics, config.ConfigAgentFlags.FlagPollInterval, config.ConfigAgentFlags.FlagReportInterval, config.ConfigAgentFlags.FlagAddressAndPort)
	}()
	select {}
